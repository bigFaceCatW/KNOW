...
//        替换原来后，返回原来的值
    ...
    @Test
    public void getSetNx() {
//        有值不操作，无值设置值
        redisTemplate.opsForValue().setIfAbsent("time","2020年12月7日10:24:34");
    }
...
//    ====================================================zset======================================
    @Test
    public void zadd(){
        redisTemplate.opsForZSet().add("zcat", 1, 12);

    }

    @Test
    public void zremove(){
        redisTemplate.opsForZSet().remove("zset",1);
    }
    @Test
    public void zedit(){
//       存在就修改分数！在原来分数上增加x分数；不参加就新增
        Double d =redisTemplate.opsForZSet().incrementScore("zset",1,1);
        System.out.println(d);
        Double d1 =redisTemplate.opsForZSet().incrementScore("zset",6,11);
    ...
    public void zscore(){
//       返回分数，如果没有就返回null
        Double d =redisTemplate.opsForZSet().score("zset",2);
        System.out.println(d);
    }
...
//       获取排名，按越小排名越高
    ...
    public void zsize(){
//       获取长度
       Long set= redisTemplate.opsForZSet().size("zset");
    }

    @Test
    public void zrange(){
//       获取内容：0 -1 表示获取全部的集合内容
        Set<?>  set =redisTemplate.opsForZSet().range("zset",0,-1); //分数小的在前面
        Set<?>  set2 =redisTemplate.opsForZSet().reverseRange("zset",0,-1);//分数大的在前面
        Set<?>  set3 =redisTemplate.opsForZSet().rangeByScore("zset",7,10);//按照分数满足条件
        System.out.println("range>>>"+set.toString());
        System.out.println("reverseRange>>>"+set2.toString());
        System.out.println("rangeByScore>>>"+set3.toString());
